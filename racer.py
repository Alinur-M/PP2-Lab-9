...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.scale(pygame.image.load(
            "./Lab_9/racer/Enemy.png"), (50, 60)) # Загрузка изображения врага
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.transform.scale(pygame.image.load(
            "./Lab_9/racer/Player1.png"), (50, 60))  # Загрузка изображения игрока
        self.rect = self.image.get_rect()
        self.rect.center = (200, 500)

    def move(self):
        klav = pygame.key.get_pressed()
        if klav[K_UP] and self.rect.top > 0:
            self.rect.move_ip(0, -9)
        if klav[K_DOWN] and self.rect.bottom < SCREEN_HEIGHT:
            self.rect.move_ip(0, 9)
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == INC_SPEED:
            SPEED += 0.25  # Увеличение скорости врагов
        if event.type == QUIT:
            pygame.quit()
            sys.exit()
    ...
    if pygame.sprite.spritecollideany(P1, coins):
        ...
        SCORE += random.randint(1, 5)  # Увеличение очков
        if SCORE > 10 and FLAG_CHECK == False:
            SPEED += 3
            FLAG_CHECK = True
